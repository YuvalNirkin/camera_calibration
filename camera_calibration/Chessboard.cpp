...
#include "camera_calibration/ChessboardQuad.h"
#include "camera_calibration/Spline.h"
    ...
    for (size_t idx = 0; idx < existingQuads.size(); ++idx)
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            if (closestCornerIdx >= 0 && minDist < FLT_MAX)
            {
                ...
                ChessboardQuadPtr newQuad = std::make_shared<ChessboardQuad>();
                newQuad->count        = 1;
                ...
                for (int j = 0; j < 4; j++)
                {
                    newQuad->corners[j] = std::make_shared<ChessboardCorner>();
                          ...
                          int dilation, bool firstRun)
{
    ...
    for (size_t idx = 0; idx < quadContours.size(); ++idx)
    {
        ...
        // Reset group ID
        q = std::make_shared<ChessboardQuad>();
        assert(contour.size() == 4);

        for (int i = 0; i < 4; ++i)
        {
            cv::Point2f pt = contour.at(i);
            ChessboardCornerPtr corner = std::make_shared<ChessboardCorner>();
            corner->pt = pt;
            q->corners[i] = corner;
        }
