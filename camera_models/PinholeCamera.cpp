...
void PinholeCamera::Parameters::writeToCfgFile(const std::string& filename) const
{
    // Open file stream
    ...
    // Write model type to file
    file << "PINHOLE" << std::endl;

    // Write data to file
    file << m_fx << " " << m_fy << " " << m_cx << " " << m_cy << " "
        << m_k1 << " " << m_k2 << " " << m_p1 << " " << m_p2 << std::endl;

    file << m_imageWidth << " " << m_imageHeight << std::endl;
                            ...
                            Eigen::Matrix<double,2,3>& J) const
{
    ...
    //double dxdmx, dydmx, dxdmy, dydmy;  // Yuval: Uninitialized???
...
// Yuval
void PinholeCamera::writeParametersToCfgFile(const std::string& filename) const
{
    m_parameters.writeToCfgFile(filename);
}
